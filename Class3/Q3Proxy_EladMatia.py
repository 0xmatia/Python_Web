...
def main():
    ...
    while True:  # loop forever (listen socket will wait until it receives a connection)
        ...
        if not (server_response[:5] == "ERROR") and (client_message.find("Russia") == -1):
            server_response = server_response[:len(server_response) - 11] + "." + server_response[len(
                server_response) - 11:]  # assemble the new request
            server_response = server_response[:16] + "Proxy" + server_response[16:]
        # modify servererror to error
        if (server_response[:6] == "SERVER") and (client_message.find("Russia") == -1):
            server_response = server_response[6:]
        # stop RUSSIA!
        if not client_message.find("Russia") == -1:
            server_response = "ERROR#\"Cannot show movies from Russia!\""
